...
const Home: NextPage = () => {
    ...
    useEffect(() => {
        if (chosenZip) {
            ...
            axios.get(`https://osfd-backup-2.herokuapp.com/tokens/tokenPreview/${chosenZip.hash}`)
                .then(
                    ({ data, status }) => {
                        setPreview(data);
                        console.log(status)
    ...
    async function upload() {
        ...
        for (const file of files) {
            ...
            axios.post('https://osfd-backup-2.herokuapp.com/tokens/zip', { hash: file.cid, name: file.name }).then(
                () => {
                    //console.log(data)
    ...
    return (
        <Box sx={{ height: "100%", p: 2 }}>
            ...
            <Box>
                ...
                <Box style={{ display: "flex", justifyContent: "center", marginBlock: 15, paddingBlock: 15, border: "3px solid black" }}>
                    <Input type="file" />
                ...
                <Box style={{ display: "flex", justifyContent: "center", marginBlock: 2, paddingBlock: 2, border: "3px solid black", overflowX: "auto" }}>
                    {zips &&
                        zips.map(
                            (zip) =>
                                <Button
                                    key={zip.hash}
                                    style={{ textAlign: "center", marginInline: 6, border: chosenZip == zip ? "2px black solid" : "none", }}
                                    variant="contained"
                                    onClick={() => { setChosenZip(zip); if (step == 0) { setStep(1) } }}
                                >
                                    {zip.name}
                                </Button>
                        )
                ...
                }
                {
                    ...
                    <Box style={{ width: "100%", display: "flex", justifyContent: "center" }}>
                        <div style={{ marginInline: 100 }}>
                            <a href={"https://bafybeic2irli7jr7zczaaweuwhbrft4g5tc4vhuvca2z2mt2tzq6u4udjm.ipfs.w3s.link"}> <b><u>example zip</u></b></a>
                        </div>
                        <div style={{ marginInline: 100 }}>
                            <a href={"https://whimsical.com/publish-2aKLEyPDbw6yZiULcquYf2"}><b><u>instructions</u></b></a>
                        </div>
                    </Box>
                }
                {
